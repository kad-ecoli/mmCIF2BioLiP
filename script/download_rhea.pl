...
{
    system("wget -q https://firedb.bioinfo.cnio.es/rest/ligand/list/$filename -O $rootdir/firedb/$filename");
}

print "download Rhea\n";
system("mkdir -p $rootdir/rhea/txt/") if (!-d "$rootdir/rhea/txt/");
system("wget -q https://ftp.expasy.org/databases/rhea/txt/rhea-reactions.txt.gz -O $rootdir/rhea/txt/rhea-reactions.txt.gz");
if (!-s "$rootdir/rhea/txt/rhea-reactions.txt.gz")
{
    system("wget -q ftp://ftp.expasy.org/databases/rhea/txt/rhea-reactions.txt.gz -O $rootdir/rhea/txt/rhea-reactions.txt.gz");
}
...
{
    system("wget -q https://ftp.expasy.org/databases/rhea/tsv/$filename -O $rootdir/rhea/tsv/$filename");
    if (!-s "$rootdir/rhea/tsv/$filename")
    {
        system("wget -q ftp://ftp.expasy.org/databases/rhea/tsv/$filename -O $rootdir/rhea/tsv/$filename");
    }
}

print "download ChEBI\n";
system("mkdir -p $rootdir/chebi/Flat_file_tab_delimited") if (!-d "$rootdir/chebi/Flat_file_tab_delimited");
system("wget -q https://ftp.ebi.ac.uk/pub/databases/chebi/Flat_file_tab_delimited/chebiId_inchi.tsv -O $rootdir/chebi/Flat_file_tab_delimited/chebiId_inchi.tsv");
if (!-s "$rootdir/chebi/Flat_file_tab_delimited/chebiId_inchi.tsv")
{
    system("wget -q ftp://ftp.ebi.ac.uk/pub/databases/chebi/Flat_file_tab_delimited/chebiId_inchi.tsv -O $rootdir/chebi/Flat_file_tab_delimited/chebiId_inchi.tsv");
}
