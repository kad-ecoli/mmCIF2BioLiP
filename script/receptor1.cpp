 ...
 * $ g++ -O3 -std=c++11 receptor1.cpp -o receptor1
 */

const char* docstring=""
"receptor1 input.pdb output.pdb\n"
"    renumber residue index in input.pdb to start from 1. output reindexed\n"
"    pdb to output.pdb\n"
"\n"
"receptor1 inputdir/ outputdir/ list\n"
"    renumber all pdb files listed by 'list' in inputdir/. output reindexed\n"
"    pdb to outputdir/\n"
;
...
size_t readList(const string &infile,vector<string>&lines)
{
    stringstream buf;
...
void receptor1(const string &infile,const string&outfile)
{
    stringstream buf;
    vector<string> lines;
    readList(infile,lines);

    /* renumber residue index */
    size_t r=0;
    size_t l;
    string prev_resi="";
    string resi="";
    ...
    {
        line=lines[l];
        if (line.size()>=54 && (StartsWith(line,"ATOM  ")
    ...
    {
        ofstream fout;
        fout.open(outfile.substr(0,outfile.size()-3).c_str());
        fout<<txt<<flush;
        fout.close();
        line="gzip -f "+outfile.substr(0,outfile.size()-3);
...
int main(int argc,char **argv)
{
    ...
    if (listfile.size()==0) receptor1(infile,outfile);
    else
    {
        vector<string> filename_list;
        readList(listfile,filename_list);
        size_t l;
        for (l=0;l<filename_list.size();l++)
            receptor1(infile+filename_list[l],outfile+filename_list[l]);
        vector<string> ().swap(filename_list);
    }
